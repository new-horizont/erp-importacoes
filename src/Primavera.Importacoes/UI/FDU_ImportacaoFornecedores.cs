...
{
    public partial class FDU_ImportacaoFornecedores : CustomForm
    {
        ClsFornecedor motor;
        public FDU_ImportacaoFornecedores()
        {
            InitializeComponent();
        }

        private void FDU_ImportacaoFornecedores_Load(object sender, EventArgs e)
        {
            try
            {
                motor = new ClsFornecedor(BSO, PSO);
            }
            catch (Exception ex)
            {
